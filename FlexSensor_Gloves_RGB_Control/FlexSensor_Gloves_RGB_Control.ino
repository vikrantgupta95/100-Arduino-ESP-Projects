/*
  ...
  Description: This Arduino code is designed for a project involving flex sensors and RGB LEDs. 
  It reads analog values from flex sensors connected to pins A0-A4, maps these values to RGB color 
  intensities, and controls RGB LEDs accordingly. The code initializes serial communication at 9600 
  baud rate and sets up the pins for sensor input and LED output. It continuously reads sensor values, 
  maps them to RGB intensities, and sets the LED colors based on the sensor readings. Additionally, it 
  includes functions for averaging sensor readings and mapping analog readings to RGB values.
